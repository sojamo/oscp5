...
public class OscNetManager implements UdpPacketListener , TcpPacketListener {
	...
	private void newTcp( ) {
		if ( _myOscProperties.listeningPort( ) > 0 ) {
			...
			_myTcpClient = new TcpClient( this , _myOscProperties.remoteAddress( ).address( ) , _myOscProperties.remoteAddress( ).port( ) , TcpClient.MODE_STREAM );
			isTcpClient = true;
		}
	}
	...
	private void newMulticast( ) {
		if ( _myOscProperties.remoteAddress( ) != null && _myOscProperties.remoteAddress( ).isvalid( ) ) {
			_myMulticast = new Multicast( this , _myOscProperties.remoteAddress( ).address( ) , _myOscProperties.remoteAddress( ).port( ) , _myOscProperties.datagramSize( ) );
			...
			try {
				...
				case ( OscProperties.UDP ):
					if ( _myOscProperties.srsp( ) ) {
						_myUdpServer.send( thePacket.getBytes( ) , _myOscProperties.remoteAddress( ).inetaddress( ) , _myOscProperties.remoteAddress( ).port( ) );

					} else {
						_myUdpClient.send( thePacket.getBytes( ) , _myOscProperties.remoteAddress( ).inetaddress( ) , _myOscProperties.remoteAddress( ).port( ) );
					}
					break;
				case ( OscProperties.TCP ):
					if ( isTcpServer ) {
	...
	public void send( final NetAddressList theList , final String theAddrPattern , final Object ... theArguments ) {
		send( new OscMessage( theAddrPattern , theArguments ) , theList );
	}

	public void send( final NetAddress theHost , final String theAddrPattern , final Object ... theArguments ) {
