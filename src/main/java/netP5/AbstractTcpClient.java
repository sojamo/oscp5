...
public abstract class AbstractTcpClient implements Runnable {
	 ...
	 *            int
	 */
	public AbstractTcpClient(final String theHost, final int thePort) {
	...
	public AbstractTcpClient(final AbstractTcpServer theTcpServer,
			...
			final int theServerPort, final int theMode) {
	...
	private void startSocket() {
		try {
			if (_mySocket == null) {
				_mySocket = new Socket(_myNetAddress.address(),
						_myNetAddress.port());
			} else {
				_myNetAddress = new NetAddress(_mySocket.getInetAddress()
						.getHostAddress(), _mySocket.getPort());
			}
			Logger.printProcess("TcpClient", "### starting new TcpClient "
					+ _myNetAddress);
	...
	private void read() {
		try {
			_myInput = new BufferedReader(new InputStreamReader(
			...
			Logger.printProcess("TcpClient.read()",
	...
	private void readline() {
		try {
			...
			Logger.printProcess("TcpClient.readline()",
					"connection has been terminated.");
			handleStatus(NetStatus.CONNECTION_TERMINATED);
			if (_myTcpServer == null) {
				handleStatus(NetStatus.SERVER_CLOSED);
